...
#### Gmailアプリパスワードの取得方法
Gmailを使ってメールを送信するには、通常のパスワードではなく「アプリパスワード」が必要です（2段階認証の有効化が必須）。

1. [Googleアカウント管理](https://myaccount.google.com/) にログイン
2. 左メニュー「セキュリティ」をクリック
3. 「2段階認証プロセス」が「オン」になっていることを確認（オフならオンにする）
4. 検索バーで「アプリ パスワード」と検索して選択
5. アプリ名に適当な名前（例: "Tech News App"）を入力して「作成」
6. 表示された16桁のパスワード（`xxxx xxxx xxxx xxxx`）をコピーして、`.env` の `GMAIL_APP_PASSWORD` に貼り付け（スペースはそのままでも削除しても可）

※ **パスワードを削除する場合**: 同じ「アプリ パスワード」画面で、削除したい・使い終わったパスワードの横にあるゴミ箱アイコン🗑️をクリックすると無効化できます。
